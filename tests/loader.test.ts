...
test('load attributes', async() => {
  ...
  expect((rows[0].category as any).id).toBe(category.id);
...
test('load with defaults', async() => {
...

  {
    ...
    expect((category.parent as any).id).toBe(1);
  }

  {
    ...
    expect((category.parent as any).id).toBe(2);
  }

  {
    ...
    expect((category.parent as any).id).toBe(3);
...
test('load many to many #2', async() => {
  ...
  expect((rows[0].user as any).id).toBe(users[0].id);
  expect((rows[1].user as any).id).toBe(users[0].id);
  expect((rows[2].user as any).id).toBe(users[1].id);
...
test('load many to many #3', async() => {
  ...
  const ids = rows.map(row => (row.category as any).id);

  {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      expect((rows[i].parent as any).id).toBe(1);
    }
  }
